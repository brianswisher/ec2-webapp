...
- When developing locally, the single command `bin/brianswisher-httpd.mv` runs your web server and takes care of serving static files
- When deploying changes (after a git push), `brianswisher-update restart` deploys changes and restarts services on your server
- Rolling back the server to an earlier version is a simple as `brianswisher-update restart v0.1.2`
